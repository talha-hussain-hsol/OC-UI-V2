...
function SystemRoutes(props) {
  ...
  const [dataFromCustomerIdentity, setDataFromCustomerIdentity] =
    useState(null);
  ...
  const getComplianceData = async () => {
    try {
      ...
      const response = await getComplianceDataAPI(
        account_id,
        identity_id,
  ...
  const getSingleAccountDetailById = async (accountId) => {
    try {
      ...
      if (response.success) {
        ...
        const attachedIdentities =
          accountDetail.attach_identities[0]?.meta?.identities;
        const faceVerificationRemarks =
          attachedIdentities?.[identity_id]?.faceVerification?.remarks;
          ...
          <Routes>
            <Route
              ...
              element={
                <IdentitySummary
                  accountData={accountData}
                  complianceDataMissing={complianceDataMissing}
                  riskAssesmentstatusData={riskAssessmentStatusData}
                  riskAssessmentStatusTab={riskAssessmentStatusTab}
                />
              }
            />
            ...
            />
            <Route
              path="/screening/:identity_id/:account_id"
              element={<IdentityScreening />}
            />
            <Route
              path="/risk-assessment/:identity_id/:account_id"
              element={<IdentityRiskAssessment />}
            />
            <Route
              path="/compliance/:identity_id/:account_id"
              element={
                <IdentityCompliance
                  getComplianceData={getComplianceData}
                  complianceDataFields={complianceDataFields}
                  dataFromCustomerIdentity={dataFromCustomerIdentity}
                  handleAlert={handleAlert}
                />
              }
            />
            <Route
              path="/report/:identity_id/:account_id"
              element={<IdentityReport />}
            />
            <Route
              path="/subscriptions/:identity_id/:account_id"
              element={<IdentitySubscriptions />}
            />
            <Route
              path="/wallets/:identity_id/:account_id"
              element={<IdentityWallets accountData={accountData} />}
            />
            <Route
              path="/face-verification/:identity_id/:account_id"
              element={
                <IdentityFaceVerification
                  getSingleAccountDetailById={getSingleAccountDetailById}
                />
              }
            />
            <Route
              ...
              element={
                <Vcip getSingleAccountDetailById={getSingleAccountDetailById} />
              }
            />
            <Route
              path="/transactions/:identity_id/:account_id"
