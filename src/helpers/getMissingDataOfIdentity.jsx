export default function getMissingDataOfIdentity(
  identityData,
  fundData,
  allFieldsData,
  isCrp,
        ...
        allFieldsData.map((items, index) => {
          ...
          if (isCrp) {
            if (customerType == identityData?.type?.toLowerCase()) {
              if (
                (allIdentityFields[item]?.value === null ||
                  allIdentityFields[item]?.value == "") &&
                items[item]?.required &&
                items[item]?.for != "root" &&
    ...
    if (isCrp) {
      ...
      if (fundRequiredDocuments) {
        fundRequiredDocuments.map((item, index) => {
          if (
            item?.documentType?.categoryId == 1 &&
            item?.documentType?.key != "OTHER" &&
            item?.documentType?.key != "FACE_VERIFICATION" &&
            item?.for != "crp" &&
            item?.entityTypeId == identityTypeId
          ) {
