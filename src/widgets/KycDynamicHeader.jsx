...
import { Col, Nav, Row, Button, Dropdown, Modal, Container, Form,  Table } from "react-bootstrap";
import { fetchDomainReport, fetchEntityReport, fetchExportDataForEntityOrIdentity, getResultSearchIdentity } from "../api/network/AdministrationApi/AdministrationApi";
import axios from "axios";
// import LoadingSpinner from "../widgets/bootstrap-component/Spinner";
import Loader from "../components/ui/loader";
import { postIdentityAttatchWithFund, postIdentityList } from "../api/network/AdministrationApi/AdministrationApi";
import FundBox from "./fund-box";
// import EntityIcon from "./../icons/entityIconSmall.svg";
import EntityIcon from '../assets/UserIcon.jpg'
...
export default function KycDynamicHeader({ ...props }) {
  ...
  const [modalShowOptionIdentities, setModalShowOptionIdentities] = useState(false);
  ...
  const forOngoingDueDiligence = checkPermissions("ON_GOING_DUE_DILIGENCE_READ");
  ...
  }, [fundConfig, params, forExpiringDocument, forPeriodicReview, forOngoingDueDiligence, navigate, hasNavigated]);
  ...
  const postIdentityAttatchWithFundApi = async (portal = "admin") => {
    ...
    const data = {
      ...
      share_holder_count: emailFieldsForJointAccount.length == 0 ? 1 : emailFieldsForJointAccount.length,
      joint_account_emails: jointAccountIsEnabled ? emailFieldsForJointAccount : [],
    };
    const response = await postIdentityAttatchWithFund(selectedIdentity?.id, data, cancelTokenSource.token);
      ...
      if (portal == "customer") {
        ...
        // return;
        {
          selectedIdentity?.type.toLowerCase() == "individual"
            ? (window.location.href = `${window.location.protocol}//${environment}customer.${domain}${port}/profile/identity/${selectedIdentity?.type.toLowerCase()}/documents/${response?.data?.identityId}/${response?.data?.accountId}?refresh=yes`)
            : (window.location.href = `${window.location.protocol}//${environment}customer.${domain}${port}/profile/identity/${selectedIdentity?.type.toLowerCase()}/organization-chart/${response?.data?.identityId}/${
  ...
  const performSearch = async () => {
    ...
    const response = await getResultSearchIdentity(searchTerm, params?.fund_id, cancelTokenSource.token);
    ...
    <>
      <Row className="justify-content-center">
        <Col xs={12} md={10} lg={8} xl={6} className="text-center">
          ...
          <p className="mb-5 text-muted">You may use the search box to find an existing customer identity. If you wish to create a new customer identity, you may click on the “Add New Customer” Button</p>
        </Col>
      </Row>
        ...
        <div className="col-sm-12">
                ...
                <div class="d-grid gap-2 col-12 mx-auto">
                  <div className="row mb-3">
                    ...
                    <div className="col-sm-12 col-md-2 text-center">
                      <button className="btn btn-outline-secondary w-100 w-md-95" disabled={searchTerm?.length > 2 ? false : true} onClick={performSearch}>
                        Search
                      </button>
                    </div>
                    <div className="col-sm-12 col-md-2 text-center">
                      <button onClick={() => setIsAddNewIdentity(true)} className="btn btn-primary w-100 w-md-95">
                        Add New Customer
                      </button>
                    </div>
                  </div>
          ...
          {clickSearchTerm ? (
            ...
            searchResults.length > 0 && (
              <div style={{ border: "1px solid white", maxHeight: "19em", overflow: "auto" }} className="table-responsive">
                  ...
                  <tbody className="list">
                    {searchResults.length > 0 ? (
                      searchResults.map((item, index) => (
                        <tr key={index} style={{ backgroundColor: isIdentitySelected(index) ? "#6d89ae" : "inherit" }}>
                          {/* <td>
                          ...
                          <td>
                            <span>{item?.createdAt ? format(new Date(item.createdAt), "MM/dd/yyyy") : "N/A"}</span>
                          </td>
                          <td>
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <FaUserPlus style={{ fontSize: "20px", color: "white" }} />
                              </div>
                            </Button>
                          </td>
                        </tr>
                      ...
                      <tr>
                        <td colSpan="6" style={{ textAlign: "center", paddingTop: "15px" }}>
                          No data found
                        </td>
                      </tr>
                    )}
          ...
          {isSetSearchResult && (
            <div className="list-group" style={{ border: "1px solid white", padding: "2px", overflow: "scroll", cursor: "pointer" }}>
      ...
      <Nav className="row align-items-center">
        ...
        <Col xs="auto">
          <Button disabled={selectedIdentity == null ? true : false} size="lg" onClick={(e) => setActiveStep(activeStep + 1)}>
            Continue
          </Button>
        </Col>
      </Nav>
    ...
    <>
      <Row className="justify-content-center">
        <Col xs={12} md={10} lg={8} xl={6} className="text-center">
          ...
          <h1 className="mb-3">Create New Customer {selectedIdentity == null ? "Entity" : "Identity"}.</h1>
          <p className="mb-3">Please complete the below particulars form</p>
        </Col>
      </Row>
      <div>
        {isAddNewIdentity && (
          <CreateIdentity isWizard={true} title={selectedIdentity == null ? "Entity" : "Identity"} selectedIdentity={selectedIdentity == null ? "{}" : selectedIdentity} handleSelectIdentity={selectedIdentity == null ? "{}" : handleSelectIdentity} />
        )}
      </div>
      <Nav className="row align-items-center">
        <Col xs="auto">
        ...
        <Col className="text-center">{selectedIdentity == null ? null : <h6 className="text-uppercase text-muted mb-0">Step {activeStep} of 2</h6>}</Col>
        <Col xs="auto"></Col>
      </Nav>
    </>
  );
            ...
            <div className="card">
              <div className="card-body">
                <div className="row">
                      ...
                      <div className="card">
                        <div className="card-body">
                          <Row className="justify-content-left">
                            <Col className="text-center">
                              <h6 className="mb-4 text-uppercase text-muted">Joint Account Emails</h6>
                            </Col>
                          </Row>
                  ...
                  <div className={jointAccountIsEnabled ? "col-12 col-md-6" : "col-12 col-md-12"}>
                    <div className="card">
                      <div className="card-body">
                        <Row className="justify-content-left">
                          <Col className="text-center">
                            <h6 className="mb-4 text-uppercase text-muted">Shareholders</h6>
                          </Col>
                        </Row>
                        {selectedIdentity ? (
                          <div className="row align-items-center">
                            ...
                            <div className="col ms-n2">
                              ...
                              <p className="small mb-0">
                                <span className="text-success"> </span> {selectedIdentity?.type}
                              </p>

                              <p className="small text-muted mb-1">
                                {selectedIdentity?.type.toLowerCase() == "corporate" ? "Country of Incorporation: " : "Citizenship: "}
                                {selectedIdentity?.meta?.data[selectedIdentity?.type.toLowerCase() + ".basic.country_of_residence_code"]?.value
                                  ? selectedIdentity?.meta?.data[selectedIdentity?.type.toLowerCase() + ".basic.country_of_residence_code"]?.value
                                  : selectedIdentity?.meta?.data[selectedIdentity?.type.toLowerCase() + ".basic.incorporate_country_code"]?.value}{" "}
                              </p>
                            </div>
                          </div>
                        ) : null}
  ...
  const renderOptionModal = (
    // <Modal size="xl" show={modalShowOption} onHide={handleCloseModal} aria-labelledby="contained-modal-title-vcenter" centered backdrop={modalShowOptionIdentities ? "static" : true}>
          <Modal size="xl" show={modalShowOption} onHide={handleCloseModal} aria-labelledby="contained-modal-title-vcenter" centered backdrop= "static">
  ...
  const renderModalOfIdentities = (
    <Modal size="lg" show={modalShowOptionIdentities} onHide={handleCloseModalIdentities} aria-labelledby="contained-modal-title-vcenter" centered backdrop= "static">
      <Modal.Header closeButton style={{ backgroundColor: "rgb(30, 70, 118)", display: "flex", justifyContent: "space-between" }}>
        <Modal.Title>
          <div>
      ...
      <Modal.Body style={{ backgroundColor: "rgb(30, 70, 118)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "2em" }}>
          <p style={{ color: "white" }}>Choose an identity from the list or create a new one:</p>
          <Button
            variant="primary"
        ...
        <div style={{ maxHeight: "20em", overflowY: "auto", border: identities?.length === 0 || identities == null ? "none" : "1px solid" }}>
          <Container fluid="lg">
            {identities == null ? (
              <LoadingSpinner animation="grow" custom={true} height="13em" />
            ) : identities.length === 0 ? (
              ...
              <Table striped hover variant="light" style={{ backgroundColor: "transparent" }}>
                <thead>
                  <tr>
                    <th style={{ position: "sticky", top: 0, background: "rgb(30, 70, 118)", color: "white" }}>Select</th>
                    <th style={{ position: "sticky", top: 0, background: "rgb(30, 70, 118)", color: "white" }}>Name</th>
                    <th style={{ position: "sticky", top: 0, background: "rgb(30, 70, 118)", color: "white" }}>Type</th>
                    <th style={{ position: "sticky", top: 0, background: "rgb(30, 70, 118)", color: "white" }}>Status</th>
                  </tr>
                </thead>
                <tbody>
                  {identities.map((identity) => (
                    <tr key={identity.id} className={`mb-2 ${selectedIdentity.id === identity.id ? "selected-row" : ""}`} onClick={() => handleRowClick(identity)}>
                      <td style={{ wordWrap: "break-word" }}>
                        <Form.Check type="checkbox" checked={selectedIdentity.id === identity.id} readOnly />
                      </td>
                      <td style={{ wordWrap: "break-word" }}>{identity.label}</td>
                      <td style={{ wordWrap: "break-word" }}>{identity.type}</td>
                      <td style={{ wordWrap: "break-word" }}>{identity.status}</td>
                    </tr>
                  ))}
                </tbody>
              </Table>
      ...
      <Modal.Footer style={{ display: "flex", justifyContent: "space-between", backgroundColor: "rgb(30, 70, 118)" }}>
        <div>
  ...
  const handleClickExportEntityData = async () => {
    setIsLoader(true);
    const response = await fetchDomainReport(params?.fund_id,false, cancelTokenSource.token);
    if (response.success) {
  ...
  return (
    <div className="card-header">
      {alertProps.variant == "success" && (
        <CustomAlert top={false} handleCloseAlert={handleCloseAlert} message={alertProps.message} variant={alertProps.variant} show={alertProps.show} hideAuto={alertProps.hideAuto} onClose={() => setAlertProps({ ...alertProps, show: false })}>
          {alertProps.message}
        </CustomAlert>
      )}
      {isLoader ? (
        ...
        <>
          <ul className="nav nav-tabs nav-tabs-sm card-header-tabs me-2">
            {fundConfig?.config?.modules?.kyc?.Accounts && (
              <li className="nav-item">
                <Link to={`/${params?.fund_id}/kyc/account/list`} className={history.pathname.indexOf("account/list") > -1 ? "nav-link active" : "nav-link"}>
                  Accounts
                </Link>
              </li>
            )}
            {fundConfig?.config?.modules?.kyc?.Expiring_Documents && forExpiringDocument && (
              <li className="nav-item">
                <Link to={`/${params?.fund_id}/kyc/expiring-document/list`} className={history.pathname.indexOf("expiring-document/list") > -1 ? "nav-link active" : "nav-link"}>
                  Documents Expiry
                </Link>
              </li>
            )}
            {fundConfig?.config?.modules?.kyc?.Periodic_Review && forPeriodicReview && (
              <li className="nav-item">
                <Link to={`/${params?.fund_id}/kyc/periodic-review/list`} className={history.pathname.indexOf("periodic-review/list") > -1 ? "nav-link active" : "nav-link"}>
                  Periodic Review
                </Link>
              </li>
            )}
            {fundConfig?.config?.modules?.kyc?.Due_Diligence && forOngoingDueDiligence && (
              <li className="nav-item">
                <Link to={`/${params?.fund_id}/kyc/due-diligence/list`} className={history.pathname.indexOf("due-diligence") > -1 ? "nav-link active" : "nav-link"}>
                  Due Diligence
                </Link>
              </li>
            )}
            {fundConfig?.config?.modules?.kyc?.Quick_Scan && (
              <li className="nav-item">
                <Link to={`/${params?.fund_id}/kyc/quick-scan/list`} className={history.pathname.indexOf("quick-scan") > -1 ? "nav-link active" : "nav-link"}>
                  Quick Scan
                </Link>
              </li>
            )}
          ...
          {history.pathname.indexOf("account/list") > -1 && forCreateCustomer ? (
            <div style={{ display: "flex", justifyContent: "space-between" ,alignItems:"center"}}>
              <div style={{ marginRight: "1em" }}>
                <Button onClick={() => setModalShowOption(true)} className="btn btn-sm btn-white">
              ...
              <div style={{ display: "flex", justifyContent: "end" }}>
                {" "}
                <Button  style={{ height: "28px", width: "200px", padding: "0" }}  onClick={() => handleClickExportEntityData()}>
      ...
      {isLoadingReport && <Loader animation="grow" custom={true} height="70vh" />}
      ...
      {showModal && (
        <Modal show={showModal} onHide={() => setShowModal(false)} backdrop= "static">
          ...
          <Modal.Body>
            ...
            {selectedIdentity?.type == "INDIVIDUAL" ? "You will be redirected to the customer portal for uploading the identity's KYC document(s)." : "You will be redirected to the customer portal for uploading the identity's Corporate Underlying Parties."}{" "}
            </div>
          </Modal.Body>
          <Modal.Footer>
