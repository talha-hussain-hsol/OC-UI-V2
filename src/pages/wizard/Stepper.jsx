...
function Stepper() {
  ...
  const [dataOfAccountSetupsState, setDataOfAccountSetupsState] = useState(dataOfAccountSetups || []);
  ...
  const handleDataUpdate = (updatedAccountSetup) => {
    ...
    const updatedArray = dataOfAccountSetupsState.map((item) => {
      if (item.fund_id === updatedAccountSetup.fund_id) {
        return { ...item, identity: updatedAccountSetup.identity,account:updatedAccountSetup.account };
      }
      return item;
    });
