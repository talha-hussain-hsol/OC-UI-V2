...
export default function DocumentModal(props) {
  ...
  const [parentDocumentFromProps, setParentDocumentFromProps] = useState(props?.requiredDocumentUploadedSelected)
  // const [parentDocumentFromProps, setParentDocumentFromProps] = useState({
  //   "key": "FORM_OF_IDENTIFICATION_HR",
  //   "tag": [
  //     "INDIVIDUAL",
  //     "AUSTRALIA",
  //     "DOCUMENT",
  ...
  //       "key": "has_issued_date",
  //       "type": "check",
  //       "title": "Has Issued Date",
  //       "enabled": true,
  //       "required": true
  //     },
  //     "has_document_number": {
  //       "key": "has_document_number",
  //       "type": "check",
  //       "title": "Has Document Number",
  //       "enabled": true,
  //       "required": true
  //     }
  //   },
  ...
  useEffect(() => {
  console.log("documentTypeSelecteddocumentTypeSelected",documentTypeSelected)
  console.log("documentTypeSelecteddocumentTypeSelected label",label)
  }, [documentTypeSelected,label]);
  ...
  useEffect(() => {
    console.log("types modalssss", parentDocumentFromProps);

    if (!isObjectEmpty(parentDocumentFromProps?.sub_docs)) {
      console.log(parentDocumentFromProps?.sub_docs, 'parentDocumentFromProps?.sub_docs')
      const dataArray = Object.keys(parentDocumentFromProps?.sub_docs)
        .filter(key => parentDocumentFromProps?.sub_docs[key]?.enabled) // Filter for enabled documents
        .map(key => ({
          key,
          ...parentDocumentFromProps?.sub_docs[key]
        }));

      console.log(dataArray, 'dataArray');
      setDocumentTypes(dataArray);
    } else {


      if (props.requiredDocumentSelected) {
        setDocumentTypes([props.requiredDocumentSelected]);


      }
      else {
        setDocumentTypes([parentDocumentFromProps]);

      }
      setDocumentTypes([parentDocumentFromProps]);
    } console.log('props.requiredDocumentSelected?.id', props.requiredDocumentSelected?.id)
    setDocumentParentId(parentDocumentFromProps?.ref_id);
  ...
  const isPassportDateValid = () => {
    const selectedDocumentType = documentTypes.find((item) => item.ref_id == documentTypeSelected);
  ...
  useEffect(() => {
    ...
    if (documentTypes[0] !== null) {
      const selectedItem = documentTypes.find((item) => item.ref_id === parseInt(documentTypeSelected));
      const selectedItemName = selectedItem ? selectedItem.title.toLowerCase() : "";
  ...
  const isFormValid = () => {
    ...
    return !(isContentTypeValid && isIssueDateValid && isExpiryDateValid && label != "");
  };
  ...
  const handleClickUploadDocument = async () => {
    ...
    let data = props.updatedDocId ? {
      ...
      // document_type_id: parentDocumentFromProps?.key ? undefined :   documentParentId ? documentParentId : documentTypeSelected,
      // sub_document_type_id: documentParentId == parseInt(documentTypeSelected) ? null : parseInt(documentTypeSelected),

      document_key: parentDocumentFromProps?.key,
      sub_document_key: parentDocumentFromProps?.key == documentTypeSelected ? null : documentTypeSelected,
      issued_date: issueDate ? issueDateToSend : issueDate,
      expiry_date: expiryDate ? format(new Date(expiryDate), "yyyy-MM-dd") : expiryDate,
      document_number: documentNumber,
      content_type: contentTypeData,
    ...
    if (respond.success == true) {
      let url = respond?.data?.data?.upload_url;
      ...
      axios
        ...
        .then(async (response) => {
          ...
          const res = await postVerifyUploadDocument(respond?.data?.data?.id, cancelTokenSource.token);
          if (res.success == true) {
                    ...
                    <div className="row" style={{ flexDirection: "column" }}>
                      <div className="col-sm-10">
                        <div className="form-group" style={{ width: "100%" }}>
                          ...
                          <Form.Control placeholder="Enter Label" type="text" defaultValue={label} onChange={(event) => { setLabel(event.target.value) }} />
                          {labelError && (<span style={{ color: 'red' }}>Label is required</span>)}
                        </div>
                      </div>
                      <div className="col-sm-10">
                        <div
                                ...
                                documentTypes.map((item) => {
                                  return (
                                    <option key={item.key} value={item.key}>
                                      {item.title}{" "} {item?.required ? "(Required)" : ""}
                                    </option>
                                  );
                                })}
                            </Form.Select>
                            {console.log(documentTypes, 'documentTypesdocumentTypesdocumentTypesdocumentTypes')}
                            <div>
                      ...
                      <div className="col-sm-10">
                        ...
                        >
                          {selectedDocumentData?.fields?.has_document_number?.enabled && (
                            <div className="form-group" style={{ width: "100%" }}>
                              <Form.Label>Document Number {selectedDocumentData?.fields?.has_document_number?.required ? "" : "(Optional)"}</Form.Label>
                      ...
                      {selectedDocumentData?.fields?.document_date?.enabled && (
                        <div className="col-sm-10">
                          <div
                            style={{
                              display: "flex",
                      ...
                      <>
                        {selectedDocumentData?.fields?.has_issued_date?.enabled && (
                          <div className="col-sm-10">
                            <div
                              style={{
                                display: "flex",
                            ...
                            >
                              <div className="form-group" style={{ width: "100%" }}>
                                <Form.Label>
                                  Issue Date
                                  {documentTypeSelected == "" ? null : documentTypes.filter((item) => item?.ref_id == documentTypeSelected)[0]?.has_issued_date ? "(Required)" : "(Optional)"}
                                </Form.Label>
                                <div
                                  className="flatpickr-container"
                                  style={{
                                ...
                                >
                                  <Flatpickr
                                    ...
                                    style={{ border: (imageData !== "" || contentTypeData !== "") && documentTypes.filter((item) => item?.ref_id == documentTypeSelected)[0]?.has_issued_date ? "2px solid orange" : "none" }}
                                  />
                                </div>
                              </div>
                            </div>
                        ...
                        {selectedDocumentData?.fields?.has_expiry_date?.enabled && (
                          <div className="col-sm-10">
                            <div
                              style={{
                                display: "flex",
                            ...
                            >
                              <div className="form-group" style={{ width: "100%" }}>
                                <Form.Label>
                                  Expiry Date
                                  {documentTypeSelected == "" ? null : documentTypes.filter((item) => item?.ref_id == documentTypeSelected)[0]?.has_expiry_date ? "(Required)" : "(Optional)"}
                                </Form.Label>
                                ...
                                >
                                  <Flatpickr
                                    ...
                                    className="flatpickr-custom"
                                    value={expiryDate}
                                    style={{ border: (imageData !== "" || contentTypeData !== "") && documentTypes.filter((item) => item?.ref_id == documentTypeSelected)[0]?.has_expiry_date ? "2px solid orange" : "none" }}
                                    onChange={(date, dateStr, instance) => {
                                      // Set the flag to true when a date is selected from the calendar modal
                                      expiryDateEnteredManually = false;
                      ...
                      <div className="col-sm-6">
                        <div className="form-group">
                          <Button disabled={label === "" || documentTypeSelected === "" || documentTypeSelected === undefined} onClick={() => handleClickUploadDocument()} variant="primary" size="lg"
