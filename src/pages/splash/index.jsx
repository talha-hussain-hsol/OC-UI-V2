...
const SplashScreen = () => {
      ...
      <div className="w-full lg:w-[40%] bg-gradient-to-r from-[#0c1f37] from-10% to-[#103649] to-90% p-8 text-white">
        ...
        <div className="flex">
          {portals.map((portal, index) => (
            <button
              key={index}
              onClick={() => setActivePortal(portal.type)} // Set active portal by type
  ...
  {entites?.response?.data?.rows
    ?.filter(row => row.entity?.type === activePortal) 
    .map((row, index) => (
        ...
        <div className="flex items-center">
          ...
          <div className="w-full">
            <a href="/##" className="font-light text-white">
              {row.entity?.title} {/* Display the entity title */}
            </a>
            <p className="text-sm text-gray-400 font-light">{row.entity?.type}</p> {/* Display the entity type */}
          </div>
        </div>
      ...
      <div
        ...
        style={{ backgroundImage: `url('https://dev-portal.one-constellation.com/img/splash.jpg')` }}
      >
        {/* Last Synced Button */}
