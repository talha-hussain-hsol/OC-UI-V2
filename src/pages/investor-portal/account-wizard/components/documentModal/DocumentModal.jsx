...
export default function DocumentModal(props) {
  ...
  const placeHolderForDate =
    region === "united-states-of-america-(USA)" ? "MM/DD/YYYY" : "DD/MM/YYYY";
  ...
  useEffect(() => {
    ...
    console.log(
      "props.requiredDocumentSelected?.id",
      props.requiredDocumentSelected?.id
    );
    props.requiredDocumentSelected &&
  ...
  const handleClickUploadDocument = async () => {
    ...
    let issueDateToSend;
    const dateFormat =
      region === "united-states-of-america-(USA)" ? true : false;
    if (dateFormat) {
      if (issueDate !== null) {
    ...
    const respond = props.updatedDocId
      ? await updateIdentityDocument(data, cancelTokenSource.token)
      : await postIdentityDocument(data, cancelTokenSource.token);
  ...
  function handleCloseExpiryModal(selectedDates, dateStr, instance) {
    ...
    const parsedIssueDate = dateFormat
      ? new Date(issueDate)
                          ...
                          >
                            {isCameraMode ? (
                              ...
                              >
                                ...
                                >
                                  <Button onClick={handleBackFromCamera}>
                                    Back
                                  </Button>
                                  {!imageData ? (
                                    <Button
                                ...
                                >
                                  ...
                                  {contentTypeData != "" ? (
                                    ...
                                    contentTypeData?.search("jpeg") !=
                                      ...
                                      >
                                        Please Select The Following Formats PNG,
                                        JPG and PDF{" "}
                                      </span>
                                    )
                                  ) : null}
                                  ...
                                  >
                                    ...
                                    <h5>
                                      ...
                                      <strong>PNG,JPG,PDF,DOCS</strong>
                                    </h5>
                                  </div>
                                </div>
                                <input
                  ...
                  <Col xs={12} md={6}>
                    <div className="row" style={{ flexDirection: "column" }}>
                      ...
                      {showDescriptionModal &&
                        selectedDocumentData?.bucket_key?.doc_note &&
                        selectedDocumentData?.bucket_key?.doc_note != "" && (
                          <div className="col-sm-10">
                            <div
                              style={{
                      ...
                      {errorExpiryDate && alertProps && (
                        <div>
                          <h2 style={{ color: "red" }}>
                            The document you're uploading has expired.{" "}
                          </h2>
                        </div>
                      )}
                      {passportDateValidity === false ? (
                        <div>
                          <h2 style={{ color: "red" }}>
                            The expiry date should be greater than 3 months from
                            the current date{" "}
                          </h2>
                        </div>
                      ) : null}
