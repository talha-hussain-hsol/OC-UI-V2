...
import { Alert, Badge, Button, Card, CloseButton, Col, Dropdown, Form, InputGroup, ListGroup, Pagination, Row, Table, Container } from "react-bootstrap";
import className from "classnames";
import { IndeterminateCheckbox, Select } from "../../components/vendor";

export default function TableComponent({ ...props }) {
  const columns = props.columns
  const { searchable = false, pagination = true } = props
  ...
  const {
  ...
  } = useTable(
    {
      ...
      initialState: {
        ...
        pageSize: pagination ? 5 : -1,
      },
      manualPagination: true, // Enable manual pagination
      pageCount: Math.ceil(data.length / (pagination ? 5 : data.length)), // Set page count based on pagination
    },
    useGlobalFilter,
    useSortBy,
    usePagination,
    useRowSelect,
    (hooks) => {
      hooks.useInstanceBeforeDimensions.push(({ state }) => {
        if (props?.pageNumberChangedCallback) {
          props.pageNumberChangedCallback(state.pageIndex);
        }
      });
    }
  );
  ...
  return (
    <>
      <Card>
        ...
        >
          ...
          <tbody className="fs-base" {...getTableBodyProps({ role: null })}>
            {page.length > 0 ? (
              ...
              <tr>
                <td colSpan='13px' style={{textAlign:"center"}}>
               No data available
                </td>
        ...
        {pagination && <Card.Footer className="d-flex justify-content-between">
